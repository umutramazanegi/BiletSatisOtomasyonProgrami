...
namespace SinemaBileti
{
    public partial class bilet : UserControl
    {
        ...
        private void FilmAfişiGoster()
        {
            SqlConnection conn = new SqlConnection(bgl.baglanti);
            conn.Open();
            SqlCommand komut = new SqlCommand("Select *from film_bilgileri where FilmAdi='" + comboFilmAdi.SelectedItem + "'", conn);
            SqlDataReader read = komut.ExecuteReader();
            while (read.Read())
            {
        ...
        private void Veritabanı_Dolu_Koltuklar()
        {
            ...
            SqlCommand komut = new SqlCommand("select *from Satis_Bilgileri where FilmAdi='" + comboFilmAdi.SelectedItem + "' and SalonAdi='" + comboSalonAdi.SelectedItem + "' and Tarih='" + comboFilmTarihi.SelectedItem + "' and Saat='" + comboFilmSeansı.SelectedItem + " '", conn);
            SqlDataReader read = komut.ExecuteReader();
            while (read.Read())
            {
                foreach (Control item in panel1.Controls)
        ...
        private void Film_Tarihi_Getir()
        {
            ...
            SqlConnection conn = new SqlConnection(bgl.baglanti);
            conn.Open();
            SqlCommand komut = new SqlCommand("select *from seans_bilgileri where FilmAdi='" + comboFilmAdi.SelectedItem + "' and SalonAdi='" + comboSalonAdi.SelectedItem + "'", conn);
            SqlDataReader read = komut.ExecuteReader();
            while (read.Read())
            {
        ...
        private void Film_Seansı_Getir()
        {
            ...
            SqlConnection conn = new SqlConnection(bgl.baglanti);
            conn.Open();
            SqlCommand komut = new SqlCommand("select *from seans_bilgileri where FilmAdi='" + comboFilmAdi.SelectedItem + "' and SalonAdi='" + comboSalonAdi.SelectedItem + "' and Tarih='" + comboFilmTarihi.SelectedItem + "'", conn);
            SqlDataReader read = komut.ExecuteReader();
            while (read.Read())
            {
                if (DateTime.Parse(read["Tarih"].ToString()) == DateTime.Parse(DateTime.Now.ToShortDateString()))
